...
class PagoProvider extends ChangeNotifier {
  ...
  late Map<String, dynamic> _compra;
...
//Metodo para crear el pago
  ...
  Future<void> deletePago(BuildContext context, int pagoId) async {
    try {
      ...
      await searchCompra(context, pagoId);

      final inventarioId = _compra['productoCompras'][0]
  ...
  Future<void> searchCompra(BuildContext context, int idPago) async {
    try {
      _isLoading = true;
      notifyListeners();

      final response = await _apiClient.get('/api/v1/compra/$idPago');
      ...
      if (response.statusCode == 200) {
        ...
        if (data is List) {
          ...
          _compra = Map<String, dynamic>.from(data);
        }

        print('Pago encontrado: $_compra');
      } else {
        throw Exception('Error al buscar la compra 1');
      }
    } catch (e) {
      print(e.toString());
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Error al buscar la compra: $e')),
      );
    }
  }
